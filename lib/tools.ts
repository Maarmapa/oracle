...
// Scenes = locations at height around the world
export const changeSceneTool = clientTool('change_scene', {
  ...
  Use when the user asks to see a different place, city, or location. 
...
// Oracle mood shift — changes color palette of the UI
export const changeMoodTool = clientTool('change_mood', {
  description: `Shift the Oracle's visual mood/aura. 
  Use when conversation tone shifts dramatically.
  Available: default (pink), prophecy (gold), warning (red), transcendent (cyan)`,
  schema: z.object({
